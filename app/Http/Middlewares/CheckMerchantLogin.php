...
use App\Model\{MerchantLogin, OperationMechantLog};
use Exception;
...
class CheckMerchantLogin implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        $admin = MerchantLog::query()->where('id', $login->id)->first();
        if (!$admin) {
            return $this->response->redirect('/index/error/message=商户登录信息有误');
        }
